...
public class SctServiceImpl {
    ...
    public Msg selectAllSctByNo(String sno, HttpServletRequest request, HttpServletResponse response) {
        ...
        return Msg.success().add("detailMessage", "success").add("data", sctMapper.selectAllSctByNo(sno));
    }
    ...
    public Msg studyCourseFromStudentToExcel(HttpServletRequest request, HttpServletResponse response){
        TokenUtil.addToken(request, response);
        return Msg.success().add("detailMessage", "success").add("data", sctMapper.studyCourseFromStudentToExcel());
    }

    //每个学生修了多少门课，用于导出excel的数据
    public Msg studentStudyCourseToExcel(HttpServletRequest request, HttpServletResponse response){
        TokenUtil.addToken(request, response);
        return Msg.success().add("detailMessage", "success").add("data", sctMapper.studentStudyCourseToExcel());
    }
    ...
    public Msg studentStudyCourse(String condition,int currentPage, int pageSize, HttpServletRequest request, HttpServletResponse response) {
        TokenUtil.addToken(request, response);
        return Msg.success()
                .add("detailMessage", "success")//sctMapper.studentStudyCourseCounts().size()
    ...
    public Msg addSct(Sct sct, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (sctMapper.judgeIsStudyByNo(sct.getSno(), sct.getTno(), sct.getCno()) != null) {
            //该门课已选
            return Msg.fail().add("detailMessage", "you have select this course");
        } else {
            //该门课未选择
            sctMapper.addSct(sct);
            return Msg.success().add("detailMessage","select success");
        }
    }

    //分页条件查看所有选课信息
    public Msg selectSctByPageAndCondition(String condition, int currentPage, int pageSize, HttpServletRequest request,
                                           HttpServletResponse response, String tno) {
        TokenUtil.addToken(request, response);
        String sno = request.getHeader("sno");
        try {
            return Msg.success()
                    .add("data",sctMapper.selectSctByPageAndCondition(condition, currentPage, pageSize, tno))
